...
local function ff(args) -- better def for find_files
  local args_str = args and argify(args) or ""
  return cmd("lua require('telescope.builtin').find_files(" .. args_str .. ")")
end

local function grep(args) -- better def for find_files
  local args_str = args and argify(args) or ""
...
local paths = {
  ...
  file_parentN = function(n) return 'require("benfleis").get_path_ancestor(nil, ' .. tostring(n) .. ')' end,
...
wk.register({
  ["<Leader>"] = {
    ...
    f = {
      ...
      ["2p"] = { ff({ cwd = paths.file_parentN(3) }), "[f]ind files CWD=[2p]arent" },
      ["3p"] = { ff({ cwd = paths.file_parentN(4) }), "[f]ind files CWD=[3p]arent" },
      ["4p"] = { ff({ cwd = paths.file_parentN(5) }), "[f]ind files CWD=[4p]arent" },
    ...
    s = {
      ...
      ["2p"] = { grep({ cwd = paths.file_parentN(3) }), "[g]rep CWD=[2p]arent" },
      ["3p"] = { grep({ cwd = paths.file_parentN(4) }), "[g]rep CWD=[3p]arent" },
      ["4p"] = { grep({ cwd = paths.file_parentN(5) }), "[g]rep CWD=[4p]arent" },
